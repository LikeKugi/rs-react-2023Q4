export const datalist = [
  {
    id: 62337,
    api_model: 'artworks',
    api_link: 'https://api.artic.edu/api/v1/artworks/62337',
    is_boosted: false,
    title: 'Francis Bacon Walking on Primrose Hill, London',
    alt_titles: null,
    thumbnail: {
      lqip: 'data:image/gif;base64,R0lGODlhBAAFAPQAADIuKkVDP0lGQU9MSFVTT3BuanRybnd1cHp3dXx6dYiGgYqIhJCPipGQiZeUkJ6cl6SinKekn6uqpa+tpwAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAACH5BAAAAAAALAAAAAAEAAUAAAUR4KQkj9Qwh0EEhQAM0II4UQgAOw==',
      width: null,
      height: null,
      alt_text: 'A work made of gelatin silver print.',
    },
    main_reference_number: '1981.273',
    has_not_been_viewed_much: false,
    boost_rank: null,
    date_start: 1963,
    date_end: 1963,
    date_display: '1963',
    date_qualifier_title: 'Made',
    date_qualifier_id: 4,
    artist_display: 'Bill Brandt\nEnglish, 1904\u20131983',
    place_of_origin: 'England',
    description: null,
    dimensions:
      'Image: 34.1 \u00d7 29.4 cm (13 7/16 \u00d7 11 5/8 in.); Paper: 38.6 \u00d7 30.3 cm (15 1/4 \u00d7 11 15/16 in.)',
    dimensions_detail: [
      {
        depth_cm: 0,
        depth_in: 0,
        width_cm: 29.4,
        width_in: 11.5749,
        height_cm: 34.1,
        height_in: 13.4252,
        diameter_cm: 0,
        diameter_in: 0,
        clarification: 'Image',
      },
      {
        depth_cm: 0,
        depth_in: 0,
        width_cm: 30.3,
        width_in: 11.9292,
        height_cm: 38.6,
        height_in: 15.1969,
        diameter_cm: 0,
        diameter_in: 0,
        clarification: 'Paper',
      },
    ],
    medium_display: 'Gelatin silver print',
    inscriptions:
      'Signed recto, lower right, below image, in black ink: "Bill Brandt"; inscribed verso, lower left, in graphite: "Bacon"',
    credit_line:
      'Purchased with funds provided by Lucia Woods Lindley and Daniel A. Lindley, Jr.',
    catalogue_display: null,
    publication_history:
      'Ferguson, Russell. 2023. "Bohemia: History of an Idea, 1950-2000." Exh. cat. Kunsthalle Praha/Hatje Cantz. p. 70.',
    exhibition_history:
      'Prague, Czech Republic, Kunsthalle Praha, "Bohemia: History of an Idea, 1950-2000," March 23-October 16, 2023.',
    provenance_text: null,
    edition: null,
    publishing_verification_level: 'Web Cataloged',
    internal_department_id: 20,
    fiscal_year: 1982,
    fiscal_year_deaccession: null,
    is_public_domain: false,
    is_zoomable: false,
    max_zoom_window_size: 843,
    copyright_notice: null,
    has_multimedia_resources: false,
    has_educational_resources: false,
    has_advanced_imaging: false,
    colorfulness: 5.5778,
    color: {
      h: 45,
      l: 52,
      s: 3,
      percentage: 0.07464844321166407,
      population: 679,
    },
    latitude: null,
    longitude: null,
    latlon: null,
    is_on_view: false,
    on_loan_display: null,
    gallery_title: null,
    gallery_id: null,
    nomisma_id: null,
    artwork_type_title: 'Photograph',
    artwork_type_id: 2,
    department_title: 'Photography and Media',
    department_id: 'PC-12',
    artist_id: 33736,
    artist_title: 'Bill Brandt',
    alt_artist_ids: [],
    artist_ids: [33736],
    artist_titles: ['Bill Brandt'],
    category_ids: ['PC-12'],
    category_titles: ['Photography and Media'],
    term_titles: [
      'gelatin silver (developing-out-paper) pr',
      'photographic paper',
      'black-and-white photography',
      'photographic process',
      'photography',
      'photograph',
      'photographic processes',
      'photographic techniques',
      'photography',
    ],
    style_id: null,
    style_title: null,
    alt_style_ids: [],
    style_ids: [],
    style_titles: [],
    classification_id: 'TM-213',
    classification_title: 'gelatin silver (developing-out-paper) pr',
    alt_classification_ids: ['TM-68', 'TM-67', 'TM-240'],
    classification_ids: ['TM-213', 'TM-68', 'TM-67', 'TM-240'],
    classification_titles: [
      'gelatin silver (developing-out-paper) pr',
      'photographic process',
      'photography',
      'photograph',
    ],
    subject_id: null,
    alt_subject_ids: [],
    subject_ids: [],
    subject_titles: [],
    material_id: 'TM-3046',
    alt_material_ids: [],
    material_ids: ['TM-3046'],
    material_titles: ['photographic paper'],
    technique_id: 'TM-12089',
    alt_technique_ids: ['TM-3937', 'TM-3977', 'TM-3978'],
    technique_ids: ['TM-12089', 'TM-3937', 'TM-3977', 'TM-3978'],
    technique_titles: [
      'black-and-white photography',
      'photographic processes',
      'photographic techniques',
      'photography',
    ],
    theme_titles: [],
    image_id: 'c001843e-62fd-b45c-8628-3d5f7b11a018',
    alt_image_ids: [],
    document_ids: [],
    sound_ids: [],
    video_ids: [],
    text_ids: [],
    section_ids: [],
    section_titles: [],
    site_ids: [],
    suggest_autocomplete_all: [
      {
        input: ['1981.273'],
        contexts: {
          groupings: ['accession'],
        },
      },
      {
        input: ['Francis Bacon Walking on Primrose Hill, London'],
        weight: 376,
        contexts: {
          groupings: ['title'],
        },
      },
    ],
    source_updated_at: '2023-11-10T16:01:47-06:00',
    updated_at: '2023-11-10T16:05:53-06:00',
    timestamp: '2023-11-10T16:45:11-06:00',
  },
];
